# c01_prepare_data_q.R ----

## Import ----
# xlsdata <- read.xls('data/data_q_in.xlsx')
xlsdata <- readxl::read_excel(path = "data/data_q_in.xlsx")
...
labelmat <- readxl::read_excel(path = "data/data_q_in.xlsx", sheet = 2)
labels_print <- labels
...
# xt   <- c(sort(seq(xt00, 0, -20)), seq(from = xt00 + 20, to = nrow(time), by = 20))
xt   <- c(sort(seq(xt00, 0, -20)), seq(from = xt00 + 20, to = nrow(time), by = 20))
