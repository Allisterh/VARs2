...
xlsdata <- readxl::read_excel(path = "data/data_m_in.xlsx")
data    <- xlsdata[, 3:ncol(xlsdata)]
labels  <- colnames(data)

## Timing and labeling ----
# Note: The problem with time is that as a data frame with just one column,
# it gets converted to a vector, which is useful for plotting.
# But as a tibble, it remains a two-dimensional object (column-vector) and
# we have to transpose it to become a vector.
# We can use time or as.vector(time) ...
time         <- xlsdata[, 1] + (1 / 12) * (xlsdata[, 2] - 1)
# labelmat     <- read.xls('data/data_m_in.xlsx', sheet = 2, header = FALSE)
labelmat     <- readxl::read_excel(path = "data/data_m_in.xlsx", sheet = 2)
labels_print <- labels
...
dataq     <- readxl::read_excel(path = "data/data_m_in.xlsx", sheet = 4)
...
xt   <- c(sort(seq(xt00, 0, -60)), seq(xt00 + 60, nrow(time), 60))
          
# output gap measures
plot(t(time), data$GDPgap, 'l', lwd = 1.5, ylab = 'Output gap', xlab = '', ylim = c(-12.5, 5))
grid()
lines(t(time), dataq3$GDPgap, col = 'blue', lwd = 2)
lines(t(time), dataq3$GDPhpgap, col = 'red', lty = 2, lwd = 2)
abline(h = 0)        
...
plot(t(time), Fs, 'l', lwd = 2, ylim = c(0, 1.3), xlab = '')
grid()  
Fs <- exp(-5*s2) / (1 + exp(-5*s2))
lines(t(time), Fs, col = 'blue', lwd = 2, lty = 2)
Fs <- exp(-1.5*s3) / (1 + exp(-1.5*s3))
lines(t(time), Fs, col = 'red', lwd = 2, lty = 3)        
...
for (vv in 1:ncol(data)) {
  plot(t(time), data[, vv], 'l', lwd = 2, xlab = '', ylab = labels_print[vv])
  grid()
}
